...
Ответь одним словом: positive, neutral или negative"""
                ...
                prompt = f"""Ты — специализированный ИИ-агент, который анализирует обращения граждан в сфере ЖКХ.
Твоя задача — точно определить **приоритет** обращения по четырём категориям:

- **critical** — аварийные ситуации, угроза жизни/здоровью, полное отсутствие воды/тепла/электричества, прорывы труб, пожары
- **high** — серьёзные проблемы, требующие срочного реагирования: частичное отключение услуг, протечки, отсутствие отопления зимой
- **medium** — важные, но не срочные вопросы: качество услуг, плановые работы, начисления, документы
- **low** — информационные запросы, консультации, общие вопросы без срочности

Примеры:
- "Прорвало трубу, затопило подвал!" → critical
- "Третий день нет горячей воды" → high
- "Вопрос по квитанции за прошлый месяц" → medium
- "Где можно узнать график уборки?" → low
